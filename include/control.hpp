...
namespace EMIRO
{
    ...
    public:
         ...
         * @see Control(std::shared_ptr<Copter> copter, std::shared_ptr<Logger> logger, std::shared_ptr<GPS> gps);
         */
        Control();
         ...
         * @param gps       GPS object to convert Copter Local degree into Global position
         *
         * @note
...
#pragma endregion
        ...
        void go(const float &x, const float &y, const float &z,
                const int &yaw, const float &pos_precision = 0.2f, const int &yaw_precision = 5);
