...
Copter::Copter() {
    ...
    is_init_frame = false;
    // traj_logger.init("Track", FileType::CSV, "x pos(m),y pos(m),z pos(m),w
    // orient(rad),x orient(rad),y orient(rad),z orient(rad)");
...
float Copter::copter_get_yaw(bool use360) {
    ...
    float _deg = atan2(2.0f * (w * z + x * y), 1.0f - 2.0f * (y * y + z * z)) *
                 180.0f / M_PI;
    if (use360)
...
WayPoint Copter::copter_calc_transition(WayPoint start_point,
    ...
    double b_angle =
        atan2(stop_point.y - start_point.y, stop_point.x - start_point.x);
