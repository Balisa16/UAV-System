...
namespace EMIRO
{
    ...
    ThreeAxisPID::ThreeAxisPID(const Position &target_point, const double &Kp,
                               const double &Ki, double Kd)
        : set_point(target_point), pid_x(Kp, Ki, Kd), pid_y(Kp, Ki, Kd),
          pid_z(Kp, Ki, Kd) {}

    void ThreeAxisPID::set_speed_limit(LinearSpeed limit_m_s)
    {
        if (limit_m_s.linear_x < 0.1f && limit_m_s.linear_y < 0.1f &&
    ...
    void ThreeAxisPID::get_control(Position &current_point,
                                   LinearSpeed &out_speed)
    {
